...
- **Genetics:** SCN5A is the **only ClinGen-curated definitively disease-causing gene** for BrS, identified in only ~20% of patients. Other implicated genes (SCN10A, CACNA1C, CACNB2, KCND3, SCN2B) have disputed ClinGen pathogenicity. SLMAP, SEMA3A, SCNN1A additionally identified by Campuzano et al. (2019). ([[sources/channelopathies-jaha-2025]], [[sources/arrhythmia-genetics-mgenetik-2025]])
...
- **Programmed ventricular stimulation (PVS):** FINGER (n=1,029) and PRELUDE (n=308) registries failed to show predictive utility. Pooled analysis (Sroubek et al., n=1,312) found OR 2.7 but with low positive predictive value. Recommended only as a "tie-breaker" in selected intermediate-risk cases (e.g., young patient with spontaneous type 1 + syncope of uncertain origin where easily induced VF would drive ICD recommendation) — not for routine use. ([[sources/brs-jaccep-2022]])
